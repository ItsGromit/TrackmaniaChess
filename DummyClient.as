namespace DummyClient {
    ...
    void ExecutePlayerMove(int fromRow, int fromCol, int toRow, int toCol) {
        ...
        if (isCapture) {
            ...
            Network::captureFrom = Network::ToAlg(fromRow, fromCol);
            Network::captureTo = Network::ToAlg(toRow, toCol);

            // Player is the attacker, so they are NOT the defender
            Network::isDefender = false;

            // Use square race mode map or fallback to random map
            if (currentRaceMode == RaceMode::SquareRace) {
                NewRaceMode::pendingCaptureRow = toRow;
                NewRaceMode::pendingCaptureCol = toCol;
                startnew(NewRaceMode::FetchSquareRaceMapWrapper);
            } else {
    ...
    void ApplyRaceResult(bool captureSucceeded) {
        ...
        if (!Network::AlgToRowCol(Network::captureFrom, fromRow, fromCol)) {
            print("[DummyClient] ERROR: Failed to parse captureFrom: " + Network::captureFrom);
            return;
        }
        if (!Network::AlgToRowCol(Network::captureTo, toRow, toCol)) {
            print("[DummyClient] ERROR: Failed to parse captureTo: " + Network::captureTo);
            return;
        }

        print("[DummyClient] ApplyRaceResult - captureSucceeded: " + captureSucceeded + ", from: " + Network::captureFrom + " to: " + Network::captureTo);
